...
static int32_t handle_input(struct android_app* app, AInputEvent* event)
{
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION)
        ...
        {
            uni_eventX = AMotionEvent_getX(event, i);
            uni_eventY = AMotionEvent_getY(event, i);
        }
...
{
    _glfwInputCursorPos(_glfw.windowListHead, uni_eventX, uni_eventY);
}
